...
export default function SettingsPanel() {
  ...
  const [fetchingStt, setFetchingStt] = useState(false);
  ...
  const fetchSttModels = useCallback(async () => {
    ...
    setFetchingStt(true);
    try {
      const models: ModelInfo[] = await invoke("fetch_stt_models", {
        provider: stt.provider, baseUrl: stt.baseUrl, apiKey: stt.apiKey,
      });
      setSttModels(models);
    } catch { setSttModels([]); }
    setFetchingStt(false);
  }, [stt.provider, stt.baseUrl, stt.apiKey]);

  const fetchLlmModels = useCallback(async () => {
    if (!llm.apiKey) return;
    setFetchingLlm(true);
    try {
      const models: ModelInfo[] = await invoke("fetch_llm_models", {
  ...
  useEffect(() => { if (stt.apiKey) fetchSttModels(); }, [stt.provider, stt.apiKey]);
  useEffect(() => { if (llm.apiKey) fetchLlmModels(); }, [llm.provider, llm.apiKey]);
          ...
          {TAB_KEYS.map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-medium transition-all cursor-pointer ${
                activeTab === tab.id ? "bg-brand-50 dark:bg-brand-900/20 text-brand-600 dark:text-brand-400"
                : "text-[#64748B] dark:text-neutral-400 hover:bg-[#F8FAFC] dark:hover:bg-neutral-800"}`}
            >
              <span>{tab.icon}</span><span>{t.settings[tab.labelKey]}</span>
            </button>
          ))}
          <div className="pt-4 mt-4 border-t border-[#F1F5F9] dark:border-neutral-800">
            <label className="block text-[11px] font-semibold text-[#94A3B8] dark:text-neutral-500 uppercase tracking-wider mb-2 px-1">Language</label>
        ...
        <div className="max-w-lg mx-auto px-8 py-8">
          ...
          {activeTab === "stt" && (
            <div className="space-y-5 animate-fade-in">
              ...
              <div><label className={labelClass}>{t.settings.provider}</label>
              ...
              <div><label className={labelClass}>{t.settings.apiKey}</label>
              ...
              <div>
                ...
                {sttModels.length > 0 ? (
                  <select className={selectClass} value={stt.model} onChange={e => setStt({ model: e.target.value })}>
                    {sttModels.map(m => <option key={m.id} value={m.id}>{m.display_name}</option>)}
                    <option value={stt.model}>{stt.model} (custom)</option>
                  </select>
                ) : (
                  <div className="flex gap-2">
                    <input type="text" className={`${inputClass} flex-1`} value={stt.model} onChange={e => setStt({ model: e.target.value })} />
                    <button onClick={fetchSttModels} disabled={fetchingStt || !stt.apiKey}
                      className="px-3 py-2 rounded-xl bg-[#F8FAFC] dark:bg-neutral-800 border border-[#E2E8F0] dark:border-neutral-700 hover:bg-brand-50 disabled:opacity-40 transition-colors cursor-pointer"
                      title="Fetch models from API">
                      <RefreshCw className={`w-4 h-4 text-[#94A3B8] ${fetchingStt ? "animate-spin" : ""}`} />
                    </button>
                  </div>
                )}
              </div>
              <div><label className={labelClass}>{t.settings.language}</label>
          ...
          {activeTab === "llm" && (
            <div className="space-y-5 animate-fade-in">
              ...
              <div>
                <label className={labelClass}>{t.settings.model}</label>
                {llmModels.length > 0 ? (
                  <select className={selectClass} value={llm.model} onChange={e => setLlm({ model: e.target.value })}>
                    {llmModels.map(m => <option key={m.id} value={m.id}>{m.display_name}</option>)}
                    <option value={llm.model}>{llm.model} (custom)</option>
                  </select>
                ) : (
                  <div className="flex gap-2">
                    <input type="text" className={`${inputClass} flex-1`} value={llm.model} onChange={e => setLlm({ model: e.target.value })} />
                    <button onClick={fetchLlmModels} disabled={fetchingLlm || !llm.apiKey}
                      className="px-3 py-2 rounded-xl bg-[#F8FAFC] dark:bg-neutral-800 border border-[#E2E8F0] dark:border-neutral-700 hover:bg-brand-50 disabled:opacity-40 transition-colors cursor-pointer"
                      title="Fetch models from API">
                      <RefreshCw className={`w-4 h-4 text-[#94A3B8] ${fetchingLlm ? "animate-spin" : ""}`} />
                    </button>
                  </div>
                )}
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className={labelClass}>{t.settings.temperature}</label>
                ...
                <div className="flex items-end">
                  <div className="w-full"><label className={labelClass}>{t.settings.model}</label></div>
                </div>
              </div>
              <div><label className={labelClass}>{t.settings.customPrompt}</label>
          ...
          {activeTab === "translate" && (
            <div className="space-y-5 animate-fade-in">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className={labelClass}>{t.settings.source}</label>
                  <select className={selectClass} value={translate.sourceLang} onChange={e => setTranslate({ sourceLang: e.target.value })}>
                    <option value="auto">{t.settings.languages.auto}</option>
                    <option value="zh">{t.settings.languages.zh}</option><option value="en">{t.settings.languages.en}</option>
                    <option value="ja">{t.settings.languages.ja}</option><option value="ko">{t.settings.languages.ko}</option>
                  </select>
                </div>
                <div><label className={labelClass}>{t.settings.target}</label>
                  <select className={selectClass} value={translate.targetLang} onChange={e => setTranslate({ targetLang: e.target.value })}>
                    <option value="en">{t.settings.languages.en}</option><option value="zh">{t.settings.languages.zh}</option>
                    <option value="ja">{t.settings.languages.ja}</option><option value="ko">{t.settings.languages.ko}</option>
                    <option value="de">{t.settings.languages.de}</option><option value="fr">{t.settings.languages.fr}</option>
